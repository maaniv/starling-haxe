...
class Quad extends Mesh
{
  ...
  override public function getBounds(targetSpace:DisplayObject, out:Rectangle = null):Rectangle
  {
    ...
    {
      if (targetSpace == parent && !isRotated)
      {
        // optimization
        {
          ...
          if (scaleX < 0)
          {
            out.width *= -1;out.x -= out.width;
          }
          if (scaleY < 0)
          {
            out.height *= -1;out.y -= out.height;
          }
        }
      }
      else
  ...
  public function readjustSize(width:Float = -1, height:Float = -1):Void
  {
    if (width <= 0)
    {
      width = (texture != null) ? texture.frameWidth:_bounds.width;
    }
    if (height <= 0)
    {
      height = (texture != null) ? texture.frameHeight:_bounds.height;
    }
