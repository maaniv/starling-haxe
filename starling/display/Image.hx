...
class Image extends Quad
{
  ...
  private var _tileGrid:Rectangle = null;
  ...
  {
    if (texture != null && _scale9Grid != null)
    {
      setupScale9Grid();
    }
    else
    {
      if (texture != null && _tileGrid != null)
      {
        setupTileGrid();
      }
      else
  ...
  private function setupScale9Grid():Void
  {
    ...
    {
      if (MathUtil.isEquivalent(_scale9Grid.height, texture.frameHeight))
      {
        absScaleX /= absScaleY;
      }
    }
      ...
      texCols:Vector<Float>, texRows:Vector<Float>):Int
  {
    ...
    var colWidthPos:Float;
    var rowHeightPos:Float;
    var colWidthTex:Float;
    var rowHeightTex:Float;
    var vertexData:VertexData = this.vertexData;
    var texture:Texture = this.texture;
