...
class AssetManager extends EventDispatcher
{
  ...
  private var _numLostTextures:Int = 0;
  private var _numRestoredTextures:Int = 0;
  private var _numLoadingQueues:Int = 0;

  private var _defaultTextureOptions:TextureOptions = null;
  private var _checkPolicyFile:Bool = false;
  private var _keepAtlasXmls:Bool = false;
  private var _keepFontXmls:Bool = false;
  private var _numConnections:Int = 0;
  private var _verbose:Bool = false;
  private var _queue:Array<Dynamic> = null;

  private var _textures:Map<String, Texture> = null;
  private var _atlases:Map<String, TextureAtlas> = null;
  private var _sounds:Map<String, Sound> = null;
  private var _xmls:Map<String, Xml> = null;
  private var _objects:Map<String, Dynamic> = null;
  private var _byteArrays:Map<String, ByteArray> = null;

  /** helper objects */
  private static var sNames:Vector<String> = new Vector();

  /** Regex for name / extension extraction from URL. */
  private static var NAME_REGEX = new EReg('([^\\?\\/\\\\]+?)(?:\\.([\\w\\-]+))?(?:\\?.*)?$', "");
  ...
  function SortNames(a:String, b:String):Int {
  ...
  public function loadQueue(onProgress:Function):Void
  {
    ...
    var xmls:Vector<Xml> = new Vector();
    var assetInfos:Array<Dynamic> = _queue.copy();
    var assetCount:Int = _queue.length;
    var assetProgress:Array<Dynamic> = [];
    var assetIndex:Int = 0;

    var loadQueueElement:Int->Dynamic->Void = null;
    ...
    var loadNextQueueElement:Void->Void = function():Void
    {
    ...
    processXmls = function():Void
    {
      ...
      haxe.Timer.delay(function() { processXml(0); }, 1);
    }
    ...
    processXml = function(index:Int):Void
    {
      ...
      haxe.Timer.delay(processXml.bind(index + 1), 1);
    }
                                    ...
                                    onProgress:Float->Void, onComplete:Void->Void):Void
  {
    ...
    function process(asset:Dynamic):Void
    {
      ...
      //else if (Starling.handleLostContext && _starling.context.driverInfo == "Disposed")
      else if (_starling.context.driverInfo == "Disposed")
      {
        log("Context lost while processing assets, retrying ...");
        haxe.Timer.delay(function():Void{ process(asset); }, 1);
      ...
      else if (Std.is(asset, ByteArray))
      {
        ...
        if (AtfData.isAtfData(bytes))
        {
          ...
          texture.root.onRestore = function():Void
          {
            ...
            loadRawAsset(rawAsset, null, function(asset:Dynamic):Void
            {
              ...
              {
                if (asset == null) throw new Error("Reload failed");
                texture.root.uploadAtfData(cast(asset, ByteArray), 0, true);
                asset.clear();
              }
              catch (e:Error)
              {
        ...
        {
          try { object = haxe.Json.parse(bytes.readUTFBytes(bytes.length)); }
          catch (e:Error)
          {
  ...
  private function loadRawAsset(rawAsset:Dynamic, onProgress:Float->Void, onComplete:Array<Dynamic>->Void):Void
  {
    ...
    {
        haxe.Timer.delay(function() { Type.createInstance(rawAsset, []); }, 1);
    }
    else if (Std.is(rawAsset, String) || Std.is(rawAsset, URLRequest))
    {
  ...
  private function getName(rawAsset:Dynamic):String
  {
    ...
    {
      name = Std.string(rawAsset);
    }
    else
    {
      if (Std.is(rawAsset, URLRequest))
      {
        name = cast(rawAsset, URLRequest).url;
      }
      else
      {
        if (Std.is(rawAsset, FileReference))
        {
