...
class RenderUtil
{
  ...
  public static function getTextureVariantBits(texture:Texture):Int
  {
    ...
    var _sw0_:Context3DTextureFormat = texture.format;
      ...
      repeat:Bool = false):Void
  {
    ...
    Starling.context_().setSamplerStateAt(sampler, wrap, filter, mipFilter);
  }
      ...
      convertToPmaIfRequired:Bool = true, tempReg:String = "ft0"):String
  {
    ...
    if (needsConversion)
    {
      if (resultReg == "oc")
      {
        // the output color register cannot use a write mask ...
        {
  ...
  public static function requestContext3D(stage3D:Stage3D, renderMode:String, profile:Dynamic):Void
  {
    ...
    {
      if (Std.is(profile, String))
      {
        profiles = [Std.string(profile)];
      }
      else
      {
        if (Std.is(profile, Array))
        {
    ...
    var onFinished:Void->Void = function():Void
    {
    ...
    requestNextProfile = function():Void
    {
      ...
      catch (error:Error)
      {
        ...
        {
          haxe.Timer.delay(requestNextProfile, 1);
        }
        else
        {
          throw error;
