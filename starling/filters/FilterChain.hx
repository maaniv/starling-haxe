...
class FilterChain extends FragmentFilter
{
  ...
  private var _filters:Vector<FragmentFilter> = null;
  ...
  public function new(args:Array<Dynamic> = null)
  {
    ...
    while (i < len)
    {
      if (Std.is(args[i], FragmentFilter))
      {
        addFilterAt(cast(args[i], FragmentFilter), i);
      }
      else
      {
  ...
  {
    for (filter/* AS3HX WARNING could not determine type for var: filter exp: EIdent(_filters) type: Vector<FragmentFilter> */ in _filters)
    {
      filter.dispose();
    }
  ...
  {
    var filter:FragmentFilter = _filters.splice(index, 1)[0];
