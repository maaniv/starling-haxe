...
class Effect
{
  ...
  private var _vertexBufferSize:Int = 0;  // in bytes
  private var _indexBuffer:IndexBuffer3D = null;
  private var _indexBufferSize:Int = 0;  // in number of indices
  private var _indexBufferUsesQuadLayout:Bool = false;

  private var _mvpMatrix3D:Matrix3D = null;
  private var _onRestore:Function = null;
  private var _programBaseName:String = null;

  // helper objects
  private static var sProgramNameCache:Map<String, Map<Int, String>> = new Map();
  ...
  public function render(firstIndex:Int = 0, numTriangles:Int = -1):Void
  {
    if (numTriangles < 0)
    {
      numTriangles = Std.int(_indexBufferSize / 3);
    }
    if (numTriangles == 0)
    {
      return;
  ...
  private function get_programName():String
  {
    ...
    var nameCache:Map<Int, String> = sProgramNameCache[baseName];
