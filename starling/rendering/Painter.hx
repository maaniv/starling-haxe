...
class Painter
{
  ...
  private static inline var STAGE3D_DATA_NAME:String = "starling.rendering.Painter.stage3D";
  ...
  private var _stencilReferenceValues:ObjectMap<Dynamic, Int> = null;
  private var _clipRectStack:Vector<Rectangle> = null;
  private var _batchCacheExclusions:Vector<DisplayObject> = null;

  private var _batchProcessor:BatchProcessor = null;
  private var _batchProcessorCurr:BatchProcessor = null;  // current  processor
  private var _batchProcessorPrev:BatchProcessor = null;  // previous processor (cache)
  private var _batchProcessorSpec:BatchProcessor = null;  // special  processor (no cache)

  private var _actualRenderTarget:TextureBase = null;
  private var _actualCulling:String = null;
  private var _actualBlendMode:String = null;

  private var _backBufferWidth:Float = 0;
  private var _backBufferHeight:Float = 0;
  private var _backBufferScaleFactor:Float = 0;

  private var _state:RenderState = null;
  private var _stateStack:Vector<RenderState> = null;
  private var _stateStackPos:Int = 0;
  private var _stateStackLength:Int = 0;

  // shared data
  private static var sSharedData:Map<String, Map<String, Dynamic>> = new Map();
  ...
  public function new(stage3D:Stage3D)
  {
    ...
    _stencilReferenceValues = new ObjectMap();
    _clipRectStack = new Vector();
  ...
  public function finishFrame():Void
  {
    ...
    }  // odd number -> alternating processors
    if (_frameID % 150 == 0)
    {
  ...
  {
    var length:Int = _batchCacheExclusions.length;
    for (i in 0...length)
    {
  ...
  public function drawFromCache(startToken:BatchToken, endToken:BatchToken):Void
  {
    ...
    if (!startToken.equals(endToken))
    {
      ...
      for (i in startToken.batchID...endToken.batchID + 1)
      {
        ...
        } else if (i == endToken.batchID) {
           subset.numVertices = endToken.vertexID - subset.vertexID;
           subset.numIndices = endToken.indexID - subset.indexID;
        } else {
          subset.numVertices = meshBatch.numVertices;
          subset.numIndices = meshBatch.numIndices;
        }
  ...
  private function applyClipRect():Void
  {
    ...
    if (clipRect != null)
    {
      ...
      {
        width = Std.int(_backBufferWidth);
        height = Std.int(_backBufferHeight);
      }
   ...
   *  @default true
   */
  private function get_cacheEnabled():Bool
  {
  ...
  private function set_frameID(value:Int):Int
  {
    _frameID = value;
    return value;
  }
  ...
  {
    return Std.int(_backBufferWidth);
  }
  ...
  {
    return Std.int(_backBufferHeight);
  }
  ...
  {
    var data:Map<String, Dynamic> = cast(sSharedData[STAGE3D_DATA_NAME]);
    if (data == null)
    {
      data = new Map();
      sSharedData[STAGE3D_DATA_NAME] = data;
    }
    return data;
  }
  ...
  {
    var programs:Map<String, Program> = cast(sharedData[PROGRAM_DATA_NAME]);
    if (programs == null)
    {
      programs = new Map();
