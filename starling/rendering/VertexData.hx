...
#if (flash || FLOAT_BYTE_ARRAY)
typedef ByteArray_ = openfl.utils.ByteArray;
#else
typedef ByteArray_ = starling.utils.FloatByteArray;
#end
...
class VertexData
{
  ...
  public var rawData(get, never):ByteArray_;
  ...
  private var _rawData:ByteArray_ = null;
  private var _numVertices:Int = 0;
  private var _format:VertexDataFormat = null;
  private var _attributes:Vector<VertexDataAttribute> = null;
  ...
  private static var sBytes:ByteArray_ = new ByteArray_();
  ...
  public function new(format:Dynamic = null, initialCapacity:Int = 32)
  {
    ...
    {
      if (Std.is(format, VertexDataFormat))
      {
        _format = format;
      }
      else
  ...
  public function getUnsignedInt(vertexID:Int, attrName:String):Int
  {
  ...
  public function setUnsignedInt(vertexID:Int, attrName:String, value:Int):Void
  {
  ...
  public function updateTinted(attrName:String = "color"):Bool
  {
    ...
    for (i in 0..._numVertices)
    {
      ...
      if (_rawData.readInt() != 0xffffffff)
      {
        _tinted = true;
        break;
      }
...
#else
      alpha = _rawData.get(alphaPos) / 255.0 * factor;
#end
      //_rawData.position = alphaPos;
      //alpha = _rawData.readByte() / 255.0 * factor;
...
#end
        //_rawData.position = alphaPos;
        //_rawData.writeByte(Std.int(alpha * 255.0));
      }
      else
      {
...
#else
//var str = "";
//var floats = _rawData.get_float32_array();
//for(i in 0...floats.length) {
  //str += floats[i] + ",";
//}
//trace(str + _rawData.length, vertexID, numVertices);
      var vertex_32_size = _vertexSize >> 2;
      var start = vertexID * vertex_32_size;
		  var count = numVertices * vertex_32_size;
      var sub_array =
        _rawData.get_float32_array().subarray(start, start + count);
      buffer.uploadFromTypedArray(sub_array);
//#else
//var str = "";
//for(i in 0...Std.int(_rawData.length / 4)) {
  //_rawData.position = i * 4;
  //str += _rawData.readFloat() + ",";
//}
//trace(str + _rawData.length, vertexID, numVertices);
      //buffer.uploadFromByteArray(_rawData, 0, vertexID, numVertices);
#end
  ...
  private static function premultiplyAlpha(rgba:Int):Int
  {
    ...
    else
    {
      ...
      var r:Int = Std.int(((rgba >> 24) & 0xff) * factor);
      var g:Int = Std.int(((rgba >> 16) & 0xff) * factor);
      var b:Int = Std.int(((rgba >> 8) & 0xff) * factor);
  ...
  private function set_numVertices(value:Int):Int
  {
    if (value > _numVertices)
    {
      ...
      for (i in 0..._numAttributes)
      {
        ...
        if (attribute.isColor)
        {
          // initialize color values with "white" and full alpha
          {
  ...
  private function get_rawData():ByteArray_
  {
    return _rawData;
  }
  ...
  private function set_format(value:VertexDataFormat):VertexDataFormat
  {
    ...
    for (a in 0...numAttributes)
    {
      ...
      if (srcAttr != null)
      {
        // copy attributes that exist in both targets
        {
          pos = tgtAttr.offset;
