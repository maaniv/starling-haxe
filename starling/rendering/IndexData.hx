...
class IndexData
{
  ...
  // helper objects
  private static var sVector:Vector<Int> = new Vector();
  ...
  public function toVector(out:Vector<Int> = null):Vector<Int>
  {
    if (out == null)
    {
      out = new Vector<Int>(_numIndices);
    }
    else
    {
  ...
  private static function getBasicQuadIndexAt(indexID:Int):Int
  {
    ...
    {
      if (posInQuad == 1 || posInQuad == 3)
      {
        offset = 1;
      }
      else
      {
        if (posInQuad == 2 || posInQuad == 5)
        {
          offset = 2;
        }
        else
